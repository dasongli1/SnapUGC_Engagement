...
from video_utils import video_transforms, volume_transforms
'''
...
def read_data(anno_file, num_frame=32, split='test', max_words=30, read_local_data=True):
    video_path = anno_file
    video_array = read_frames_decord(video_path, num_frames=num_frame, sample='middle')
    # golden_captions = None
    video_array = video_test_transform(video_array)
    return video_array
def read_data2(anno_file, num_frame=32, split='test', max_words=30, read_local_data=True):
    video_path = anno_file
    video_array = read_frames_decord(video_path, num_frames=num_frame, sample='middle', all_frames=True)
    # golden_captions = None
    video_array = video_test_transform(video_array)
    return video_array