...
def create_dataset(dataset, config, epoch=None):
    ...
    if dataset=='pretrain':
        ...
        return train_dataset, val_dataset
    elif dataset == "video_caption2":
        train_dataset = video_caption_dataset2(config["train_file"], video_train_transform, config["video_root"],
            num_frames=config["model_num_frames"], split="train", read_local_data=True)
        val_dataset = video_caption_dataset2(config["test_file"], video_test_transform, config["video_root"],
            num_frames=config["model_num_frames"], split="test", read_local_data=True)
        return train_dataset, val_dataset
