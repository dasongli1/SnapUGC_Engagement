...
from models.modeling_mplug2 import BertConfig, BertModel, BertPrefixModel, FusionModel
from models.visual_transformers import initialize_clip, initialize_clip_video
...
class MPLUG2(nn.Module):
    def __init__(self,                 
        ...
        self.visual_encoder, _ = initialize_clip_video(config, T=config['model_num_frames'], use_checkpoint=config.get('use_checkpoint', True)) 
    ...
    def forward(self, image, answer=None, train=True, out_size=5, scst=False, alpha=0):
        if(scst):
        ...
        image_output = self.text_encoder(encoder_embeds=image_embeds,
                                              attention_mask=image_atts,
                                              encoder_hidden_states=image_embeds,
                                              encoder_attention_mask=image_atts,
                                              return_dict=True,
                                              mode='v_module')
        image_embeds = image_embeds + image_output.last_hidden_state
        image_embeds = self.visn_emb_dropout(self.visn_emb_norm(image_embeds))
        # simage_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
        # return image_embeds
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
        if train:               
    ...
    def forward_feature(self, image, answer=None, train=True, out_size=5, scst=False, alpha=0):
        if(scst):
            return self.beam_search(image, answer, train=True,out_size=out_size)
        image = image.to(dtype=next(self.parameters()).dtype) 
        image_embeds = self.visual_encoder.visual(image, skip_last_layer=True, use_checkpoint=self.use_checkpoint)
    ...
    def module_setting(self, config):
        self.num_frames = config["model_num_frames"]
        ...
        self.visn_emb_norm = nn.LayerNorm(self.text_width, eps=1e-12)
        # self.visn_emb_dropout = nn.Dropout(self.config_encoder.hidden_dropout_prob)
        self.visn_emb_dropout = nn.Dropout(0.0)
        # print(self.config_encoder.hidden_dropout_prob)
        # exit(0)
        self.large = False
    ...
    def beam_search(self, image, answer=None, train=True, out_size=5):
        ...
        image_embeds = image_embeds + image_output.last_hidden_state
        image_embeds = self.visn_emb_dropout(self.visn_emb_norm(image_embeds))
        image_embeds, image_atts = self._flatten_time(image_embeds, image.shape[0])
